...
export async function releaseEscrowToHost(escrowId: string, bookingId: string) {
  try {
    ...
    const booking = await sql(`SELECT status FROM bookings WHERE id = $1`, [
      bookingId,
    ]);
...
export async function refundEscrowToRenter(
  escrowId: string,
  bookingId: string,
  reason: string,
) {
  try {
    ...
    if (payment[0].flutterwave_transaction_id) {
      const refund = await refundFlutterwavePayment(
        payment[0].flutterwave_transaction_id.toString(),
      );
...
export async function resolveDispute(
  ...
  try {
    ...
    let renterAmount = 0;
    const payment = await sql(
      `SELECT amount FROM escrow_transactions WHERE id = $1`,
      [escrowId],
    );
    ...
    await sql(
      `UPDATE escrow_transactions 
       ...
       WHERE id = $4`,
      customAmount
        ? [resolution, customAmount.host, customAmount.renter, escrowId]
        : [resolution, escrowId],
    );

    // Mark dispute as resolved
    await sql(
      `UPDATE disputes SET status = 'resolved', resolved_at = NOW() WHERE escrow_transaction_id = $1`,
      [escrowId],
    );
