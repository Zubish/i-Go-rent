  ...
  const filteredListings = useMemo(() => {
    return listings.filter((listing) => {
      ...
      const matchesArea =
        ...
        listing.vendorArea.toLowerCase().includes(area.toLowerCase());
    ...
    <main className="min-h-screen bg-[#f7fbfb]">
      <section className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[0.85fr_1.15fr] lg:items-end">
          <div>
            ...
            <h1 className="text-4xl font-semibold tracking-normal text-slate-950">
              Find rentals with deposit protection.
            </h1>
            <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600">
              Search event equipment, transport, and creator gear. Every
              checkout shows rental fee, security deposit, delivery option, and
              virtual escrow status.
            </p>
          </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          ...
          {startDate && endDate && (
            <Badge variant="outline">Dates selected</Badge>
          )}
        </div>
          ...
          {filteredListings.map((listing) => (
            ...
            >
              ...
              <div className="p-5">
                ...
                <Link href={`/listings/${listing.id}`}>
                  <h2 className="mt-4 text-xl font-semibold tracking-normal hover:text-teal-700">
                    {listing.title}
                  </h2>
                </Link>
        ...
        {!loading && filteredListings.length === 0 && (
          <Card className="mt-8 rounded-lg border-slate-200 bg-white p-10 text-center">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Try a broader Lagos area or another category.
            </p>
          </Card>
        )}
      </section>
