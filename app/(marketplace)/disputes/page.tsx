  ...
  return (
    <main className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-4xl mx-auto px-4">
        ...
        <Suspense fallback={<Card className="p-8 text-center">Loading disputes...</Card>}>
      ...
      {disputes.map((dispute) => (
        <Card key={dispute.id} className="p-6">
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="text-lg font-semibold capitalize">{dispute.reason.replace(/_/g, " ")}</h3>
              <p className="text-sm text-gray-600 mt-1">Booking #{dispute.booking_id.slice(0, 8)}</p>
            </div>
            <Badge variant={dispute.status === "resolved" ? "default" : "secondary"}>{dispute.status}</Badge>
          </div>

          <p className="text-gray-700 mb-4">{dispute.description}</p>

          <div className="flex justify-between items-center">
            <span className="font-semibold text-lg">₦{(dispute.claim_amount || 0).toLocaleString()}</span>
            <Link href={`/disputes/${dispute.id}`} className="text-blue-600 hover:text-blue-700">
              View Details →
            </Link>
          </div>
        </Card>
