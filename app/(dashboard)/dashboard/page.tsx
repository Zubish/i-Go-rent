            ...
            <Card className="rounded-lg border-slate-200 bg-white p-6">
              ...
              <form onSubmit={handleCreateListing} className="mt-5 space-y-4">
                ...
                <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
                  <p className="text-sm font-semibold text-slate-900">Condition contract</p>
                  <p className="mt-1 text-xs leading-5 text-slate-600">
                    This record is shown to renters before checkout and saved on the booking for dispute review.
                  </p>
                ...
                <Button disabled={!vendorKyc.canList} className="w-full bg-teal-500 text-white hover:bg-teal-600">
                  Save listing
                </Button>
              </form>
              {createdListingId && (
...
function KycPanel({
  ...
  return (
    <div className={`mt-4 rounded-lg border p-4 ${isClear ? "border-teal-200 bg-teal-50" : "border-amber-200 bg-amber-50"}`}>
      <div className="flex items-start gap-3">
        {isClear ? <ShieldCheck className="mt-0.5 size-5 text-teal-700" /> : <AlertTriangle className="mt-0.5 size-5 text-amber-700" />}
        <div>
          <div className="flex flex-wrap items-center gap-2">
            <p className={isClear ? "font-semibold text-teal-950" : "font-semibold text-amber-950"}>{title}</p>
            <Badge className={isClear ? "bg-white text-teal-700" : "bg-white text-amber-700"}>{status}</Badge>
          </div>
          <p className={`mt-2 text-sm leading-6 ${isClear ? "text-teal-900" : "text-amber-950"}`}>
            {isClear ? successCopy : blockedCopy}
          </p>
          {!isClear && (
            <p className="mt-1 text-xs text-amber-900">Missing: {missing.join(", ")}</p>
          )}
        </div>
      </div>
    </div>
