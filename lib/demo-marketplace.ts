...
export function getKycStatus(user: DemoUser | null | undefined, role: UserRole = user?.role || "renter") {
  if (!user) {
    return {
      level: 0,
      label: "No profile",
  ...
  if (role === "vendor") {
    ...
    const level = hasPhone && hasNin && hasBvn ? (hasCac && hasBusiness ? 2 : 1) : 0

    return {
      level,
      label: level >= 2 ? "Business Verified" : level >= 1 ? "Basic Verified" : "Vendor Draft",
      canBook: false,
      canList: level >= 1,
      canDispatch: false,
      missing,
  ...
  if (role === "logistics") {
    ...
    const level = hasPhone && hasNin && hasBvn && hasLicense && hasVehicle && hasPlate ? 1 : 0
  ...
  if (!hasPhone) missing.push("Phone verification")
  if (!hasNin) missing.push("NIN")
  const level = hasPhone && hasNin && hasBvn ? 3 : hasPhone && hasNin ? 2 : hasPhone ? 1 : 0

  return {
    level,
    label: level >= 3 ? "Renter Level 3" : level >= 2 ? "Renter Level 2" : level >= 1 ? "Renter Level 1" : "Renter Level 0",
    canBook: level >= 2,
    canList: false,
    canDispatch: false,
    missing,
...
export const seedListings: DemoListing[] = [
  {
    ...
    condition: "Excellent",
    knownDefects: "Minor scuff marks on speaker stands; mixer and microphones are fully functional.",
    accessories: "2 powered speakers, mixer, 2 wireless microphones, stands, XLR cables, power cables.",
    usageLimits: "Indoor or covered outdoor use only. Not for rain exposure or generator overload.",
  ...
  },
  {
    ...
    condition: "Excellent",
    knownDefects: "Small cosmetic scratch on monitor casing; camera body and lenses tested clean.",
    accessories: "Camera body, 2 lenses, tripod, gimbal, monitor, LED panel, charger, batteries.",
    usageLimits: "No beach shoots, rain exposure, or unsupervised overnight outdoor use.",
  ...
  },
  {
    ...
    accessories: "Professional driver, spare tire, phone support, agreed fuel policy.",
    usageLimits: "Lagos routes only unless vendor approves interstate movement.",
  ...
  },
  {
    ...
    condition: "Good",
    knownDefects: "Some chairs have visible wear; canopies are waterproof but lightly stained.",
...
export function createConditionSnapshot(listing: DemoListing): DemoConditionSnapshot {
  return {
