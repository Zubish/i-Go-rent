...
export function getAllListings() {
  ...
  return [
    ...custom,
    ...seedListings.filter((listing) => !customIds.has(listing.id)),
  ];
}
...
export function getAllLogisticsProviders() {
  ...
  return [
    ...custom,
    ...seedLogisticsProviders.filter((provider) => !customIds.has(provider.id)),
  ];
}
...
export function getLogisticsProviderForUser(userId?: string) {
  ...
  return (
    getAllLogisticsProviders().find((provider) => provider.id === userId) ||
    null
  );
}
...
function saveLogisticsProviderFromUser(user: DemoUser) {
  const provider: DemoLogisticsProvider = {
    id: user.id,
    providerName:
      user.businessName || `${user.firstName} ${user.lastName} Logistics`,
  ...
  writeJson(logisticsProvidersKey, [
    provider,
...
export function createVendorListing(input: {
  ...
  const listing: DemoListing = {
    ...
    included: [
      "Vendor confirmed item",
      "Pickup checklist",
      "Escrow-backed deposit",
    ],
    available: true,
  };
...
function providerMatchesListing(
  provider: DemoLogisticsProvider,
  listing: DemoListing,
) {
  const pickupText =
    `${listing.vendorArea} ${listing.location} ${listing.deliveryArea}`.toLowerCase();
  return provider.coverageAreas.some((area) =>
    pickupText.includes(area.toLowerCase()),
  );
}
...
export function getSuggestedLogisticsProvider(listing: DemoListing) {
  ...
  return (
    providers.find((provider) => providerMatchesListing(provider, listing)) ||
    providers[0] ||
    null
  );
}
...
export function createDemoBooking(input: {
  ...
  const provider =
    input.deliveryType === "igo-logistics"
      ? getSuggestedLogisticsProvider(input.listing)
      : null;
...
export function markReturnedAndInspected(id: string) {
  const updated = getBookings().map((booking) =>
    booking.id === id
      ? { ...booking, escrowStatus: "deposit_refunded" as const }
      : booking,
  );
...
export function getBookingsForLogisticsProvider(providerId?: string) {
  ...
  return getBookings().filter(
    (booking) => booking.dispatch?.provider.id === providerId,
  );
}
